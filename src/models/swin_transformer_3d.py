...
class WindowAttention3D(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., earth_position=False, window_partition_shape=(3, 10, 10)):
        ...
        if self.earth_position:
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(window_partition_shape[0], window_partition_shape[1], (window_size[0] ** 2) * (window_size[1] ** 2) * (2 * window_size[2] - 1), num_heads))  # Nd, Nh, (Wd^2 * Wh^2 * 2*Ww-1), nH
        else:
            self.relative_position_bias_table = nn.Parameter(
    ...
    def _construct_absolute_index(self):
        ...
        relative_position_index = torch.cat([relative_coords_pl, relative_coords_lat, relative_coords_lon], dim=-1).sum(-1)  # [Wd*Wh*Ww, Wd*Wh*Ww] --> [0, Wd*Wd*Wh*Wh*(2Ww-1) -1]
        return relative_position_index
    ...
    def forward(self, x, mask=None, window_partition_shape=None):
        ...
        if self.earth_position:
            ...
            relative_position_bias = self.relative_position_bias_table[:window_partition_shape[0], :window_partition_shape[1], self.relative_position_index[:N, :N].reshape(-1)].reshape(
                1, window_partition_shape[0], window_partition_shape[1], 1, N, N, self.num_heads)  # B, wps0, wps1, wps2, Wd*Wh*Ww,Wd*Wh*Ww,nH
            relative_position_bias = torch.broadcast_to(relative_position_bias, (B, window_partition_shape[0], window_partition_shape[1], window_partition_shape[2], N, N, self.num_heads))
            relative_position_bias = relative_position_bias.permute(0, 1, 2, 3, 6, 4, 5).contiguous()  #  B, wps0, wps1, wps2, nH, Wd*Wh*Ww,Wd*Wh*Ww
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        attn_windows = self.attn(x_windows, mask=attn_mask, window_partition_shape=window_partition_shape)  # B*nW, Wd*Wh*Ww, C
        # merge windows
...
class SwinTransformer3D(nn.Module):
    ...
    def _freeze_stages(self):
        ...
        #     self.patch_embed2d.eval()
        #     for param in self.patch_embed2d.parameters():
        #         param.requires_grad = False
    ...
    def forward(self, x, posi=None, level=None, t=None, x_bc=None):
        """Forward function."""
...
if __name__ == '__main__':
    ...
    model.cuda()
    x0 = torch.rand(1, 4, 241, 281).cuda()

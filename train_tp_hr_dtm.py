...
def train_one_epoch(rank, model, train_loader, loss_func, optimizer, scheduler, writer, cfg, epoch):
    ...
    #     model.to(rank)
    print(f"total iter {iters}")
    for i, data in enumerate(train_loader): 
        timestamp, input, x_tp, target = data
            
        input = pre_processing(input, rank, cfg.model.name)
        target = target.to(rank) 
        x_tp = x_tp.to(rank)
   
        # forward
        output = model(input, x_tp)
        # calculate loss
        loss = loss_func(output, target)    
        print(f"ep {epoch} , iter {i}, loss {loss.detach()}")  
        ...
        if i % cfg.hyper_params.verbose_step == 0 and (epoch-1) % 2==0:
            ...
            if (cfg.model.mp and dist.get_rank()==0) or not cfg.model.mp:
                ...
                plot_train_result(f"{img_path}/tp_era5_{ymdh}", 
                                  "radar",
                                  output.detach().cpu().numpy()[0, 0, ::-1]*55, 
                                  target.detach().cpu().numpy()[0, 0, ::-1]*55,
                                  region='cmpa_5km')
